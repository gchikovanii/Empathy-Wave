...
namespace EmphatyWave.Web.Services.Accounts.Registrations
{
    public class RegisterService(HttpClient httpClient)
    {
        private readonly HttpClient _httpClient = httpClient;
        public async Task<ResultOrValue<string>> RegisterUser(RegisterModel registerModel)
        {
            var response = await _httpClient.PostAsJsonAsync("https://localhost:7481/api/Account/register", registerModel);
